...
export const LiveCodeStream: React.FC<LiveCodeStreamProps> = memo(({ actions, isRunning, terminalWidth = 80 }) => {
  ...
  if (isRunning) {
    ...
    if (currentAction) {
      contentLines.push(
        <Box key="current">
          <Text color={getActionColor(currentAction.type)} bold>{getActionLabel(currentAction.type)} </Text>
          <Text color="white">{formatTarget(currentAction.target, boxWidth - 15)}</Text>
        </Box>
      );
    } else {
      contentLines.push(<Text key="current" color="gray">Starting...</Text>);
    }
    
    // Line 2: Separator
    contentLines.push(<Text key="sep" color="gray" dimColor>{'─'.repeat(boxWidth - 4)}</Text>);
    
    // Line 3: File changes stats
    const changesParts: string[] = [];
    if (stats.filesCreated > 0) changesParts.push(`+${stats.filesCreated} created`);
    if (stats.filesEdited > 0) changesParts.push(`~${stats.filesEdited} edited`);
    if (stats.filesDeleted > 0) changesParts.push(`-${stats.filesDeleted} deleted`);
    
    if (changesParts.length > 0) {
      contentLines.push(
        <Box key="changes">
          <Text color="cyan">Files: </Text>
          <Text color="green">{changesParts.join(' ')}</Text>
        </Box>
      );
    } else {
      contentLines.push(<Text key="changes" color="gray" dimColor>Files: no changes yet</Text>);
    }
    
    // Line 4: Other stats
    const otherParts: string[] = [];
    if (stats.filesRead > 0) otherParts.push(`${stats.filesRead}R`);
    if (stats.commands > 0) otherParts.push(`${stats.commands}C`);
    if (stats.searches > 0) otherParts.push(`${stats.searches}S`);
    if (stats.totalLinesWritten > 0) otherParts.push(`${stats.totalLinesWritten}L`);
    
    contentLines.push(
      <Box key="other">
        <Text color="cyan">Stats: </Text>
        <Text color="gray">{otherParts.length > 0 ? otherParts.join(' | ') : 'gathering...'}</Text>
      </Box>
    );
    
    // Line 5: Errors if any
    if (stats.errors > 0) {
      contentLines.push(<Text key="errors" color="red">{stats.errors} error(s)</Text>);
    } else {
      contentLines.push(<Text key="errors" color="gray" dimColor> </Text>);
    }
    
    // Line 6: Help text
    contentLines.push(<Text key="help" color="gray" dimColor>Press Esc to stop</Text>);
    
  } else {
    // FINISHED STATE
    // Line 1: Complete header
    contentLines.push(
      <Text key="header" color={stats.errors > 0 ? 'red' : 'green'} bold>
        {stats.errors > 0 ? 'Completed with errors' : 'Completed'} ({stats.totalActions} actions)
      </Text>
    );
    ...
    if (changesParts.length > 0) {
      ...
      contentLines.push(<Text key="changes" color="gray">Files: no changes made</Text>);
    }
    ...
    if (stats.totalLinesWritten > 0) {
      contentLines.push(
        <Box key="lines">
          <Text color="cyan">Lines: </Text>
          <Text color="white">{stats.totalLinesWritten} written</Text>
        </Box>
      );
    } else {
      contentLines.push(<Text key="lines" color="gray" dimColor> </Text>);
    }
    
    // Line 5: Other stats
    const otherParts: string[] = [];
    if (stats.filesRead > 0) otherParts.push(`${stats.filesRead} read`);
    if (stats.commands > 0) otherParts.push(`${stats.commands} commands`);
    if (stats.searches > 0) otherParts.push(`${stats.searches} searches`);
    
    if (otherParts.length > 0) {
      contentLines.push(<Text key="other" color="gray" dimColor>Also: {otherParts.join(', ')}</Text>);
    } else {
      contentLines.push(<Text key="other" color="gray" dimColor> </Text>);
    }
    
    // Line 6: Errors
    if (stats.errors > 0) {
      contentLines.push(<Text key="errors" color="red">{stats.errors} error(s) occurred</Text>);
    } else {
      contentLines.push(<Text key="errors" color="gray" dimColor> </Text>);
    }
  }
  ...
  while (contentLines.length < FIXED_HEIGHT) {
    contentLines.push(<Text key={`pad-${contentLines.length}`} color="gray" dimColor> </Text>);
  }
  ...
  return (
    <Box 
      flexDirection="column" 
      borderStyle="round" 
      borderColor={isRunning ? 'cyan' : (stats.errors > 0 ? 'red' : 'green')}
      paddingX={1}
      marginBottom={1}
      width={boxWidth}
    >
...
const formatTarget = (target: string, maxLen: number = 50): string => {
